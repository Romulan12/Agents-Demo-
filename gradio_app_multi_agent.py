...
async def extract_topic_from_query(query: str, api_key: str) -> str:
    """
    Extract the main research topic from a user query using LLM.
    ...
    Returns:
        Extracted topic as a concise phrase (2-4 words)
    """
    from llama_index.llms.openai import OpenAI
    import httpx
    
    # Create HTTP clients with SSL bypass
    ...
    prompt = f"""Extract the main research topic from this query.
Return ONLY the topic as a short phrase (2-4 words), nothing else.

Examples:
Query: "What are the latest developments in computer vision?"
Topic: computer vision

Query: "Explain transformer architectures for NLP"
Topic: transformer architectures

Query: "Compare RAG approaches in the papers"
Topic: RAG systems
...
async def _web_search_fallback(user_query: str, api_key: str, show_reasoning: bool):
    """
    Simple web search fallback using tools directly.
    Avoids FunctionAgent complexity and uses direct tool calls.
    """
    ...
    if show_reasoning:
        print(f"📊 Found search results, synthesizing answer...")
    
    # Use LLM to synthesize answer from search results
    prompt = f"""Based on these web search results, provide a comprehensive answer to the question.
...
Please provide a detailed, well-structured answer. Include relevant information from the search results and cite sources where appropriate."""
...
def process_request(
    ...
    try:
        ...
        progress(0.05, desc="Router Agent analyzing your request...")
        
        router = RouterAgent(openai_api_key=api_key)
        routing_decision = asyncio.run(router.route(user_query))
        
        # Display routing decision
        print(router.explain_routing(routing_decision, user_query))
        
        # Map agent decision to intent
        intent = "blog" if routing_decision["agent"] == "blog_writer" else "question"
        
        progress(0.1, desc=f"Routing to {routing_decision['agent'].replace('_', ' ').title()}...")
        ...
        if intent == "blog":
            ...
            output = []
            output.append("🤖 ROUTER AGENT DECISION")
            output.append("=" * 60)
            output.append(f"Selected Agent: {routing_decision['agent'].upper().replace('_', ' ')}")
            output.append(f"Confidence: {routing_decision['confidence']:.0%}")
            output.append(f"Reasoning: {routing_decision['reasoning']}")
            output.append("")
            ...
            if download_arxiv:
                # Determine topic: use manual if provided, otherwise auto-extract
                if arxiv_topic and arxiv_topic.strip():
                    topic = arxiv_topic
                    progress(0.28, desc=f"Using specified topic: '{topic}'...")
                else:
                    progress(0.28, desc="Extracting topic from your query...")
                    topic = asyncio.run(extract_topic_from_query(user_query, api_key))
                    progress(0.29, desc=f"✅ Detected topic: '{topic}'")
                
                progress(0.3, desc=f"Downloading {num_papers} papers about '{topic}'...")
            ...
            # Check if we have papers - if not, use web search fallback
            if not papers:
                progress(0.5, desc="No papers available. Using web search to answer your question...")
                
                print("\n" + "="*60)
                print("⚠️  NO PAPERS PROVIDED - USING WEB SEARCH FALLBACK")
                print("="*60)
                
                progress(0.6, desc="Setting up web search agent...")
                progress(0.7, desc="Searching the web for answers...")
                
                # Use async helper function for web search
                response = asyncio.run(_web_search_fallback(user_query, api_key, show_reasoning))
                ...
                output.append(f"Reasoning: {routing_decision['reasoning']}")
                output.append("")
                output.append("⚠️  WEB SEARCH FALLBACK ACTIVATED")
                output.append("=" * 60)
                output.append("No research papers were provided, so I used web search to answer your question.")
                output.append("")
                output.append("✅ ANSWER FROM WEB SEARCH")
                output.append("=" * 60)
                output.append("")
            ...
            # Format output with routing decision and reasoning trace
            output = []
            output.append("🤖 ROUTER AGENT DECISION")
            output.append("=" * 60)
            output.append(f"Selected Agent: {routing_decision['agent'].upper().replace('_', ' ')}")
            ...
            # Add ReAct output
            output.append(format_react_output(result, show_trace=show_reasoning))
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                arxiv_topic = gr.Textbox(
                    label="Topic for arXiv papers (optional)",
                    placeholder="Leave empty to auto-detect from your query",
                    info="Override: Specify a custom topic if auto-detection isn't accurate",
                    visible=False
                )
...
""",
        ...
        **🤖 Router Agent (LLM-Powered Routing):**
        - Uses AI reasoning to detect your intent
        - Analyzes query context and expected output
        - Provides confidence scores and explanations
        - No hardcoded rules - true autonomous decision-making!
